...
for i in range(20):
    print(i)
    d = i+1
    X, X_test, y, y_test = generateData(n, d, 10, i)
    ...
    x1.append(mean_squared_error(np.dot(X,quantize(new_best_weights.T)),y))
    y1.append(count)
    ...
    #while count != args.count:
        ...
        if best_error == mse2:
            new_best_weights = quantize(beta)
    x2.append(mean_squared_error(np.dot(X,quantize(new_best_weights.T)),y))
    y2.append(count)
    ...
    while count != 500000:
        ...
        if best_error == mse2:
            new_best_weights = quantize(beta)
    x3.append(mean_squared_error(np.dot(X,quantize(new_best_weights.T)),y))
    y3.append(count)
    ...
    while count != 500000:
        ...
        if best_error == mse2:
            new_best_weights = quantize(beta)
    x4.append(mean_squared_error(np.dot(X,quantize(new_best_weights.T)),y))
    y4.append(count)

df = pd.DataFrame(list(zip(x0,x1,y1,x2,y2,x3,y3,x4,y4)),
               columns =['x0', 'x1', 'y1','x2','y2','x3','y3','x4','y4'])

df.to_csv('experiments3.csv')